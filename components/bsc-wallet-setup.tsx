...
export function BscWalletSetup() {
  const [copied, setCopied] = useState(false)

  const bscAddress = "0xa85BAC140e091e5b74c235F666e8C9849a7BBA55"

  const copyAddress = async () => {
    try {
      await navigator.clipboard.writeText(bscAddress)
      setCopied(true)
  ...
  return (
    <Card className="mb-8">
      <CardHeader>
        ...
        <CardDescription>Your Binance Smart Chain wallet address for receiving USDT</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
          <div className="flex-1">
            <p className="text-sm font-medium text-gray-600 mb-1">Receiving Address (BEP20)</p>
            <p className="font-mono text-sm break-all">{bscAddress}</p>
          </div>
        ...
        <div className="p-3 bg-yellow-50 rounded-lg border border-yellow-200">
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-yellow-800">
              <p className="font-medium mb-1">Network Information:</p>
